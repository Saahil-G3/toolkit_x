...
class Normality():
    ...
    def __init__(self): 
        self._implemented_tests = ['shapiro', 'ks', 'anderson', 'dagostino', 'jb', 'lilliefors']

    def set_data(self, data, alpha=0.05, ddof=1):
        assert len(data.shape) == 1, "Input data must be a 1-dimensional array (single row)."
    ...
    def get_normality_report(self, test_type):
        ...
        if test_type not in self._implemented_tests:
            raise ValueError(f"Test not implemented, must be one of {self._implemented_tests}")
    ...
    def _anderson(self):
        ...
        significance_index = next(i for i, val in enumerate(anderson_result.significance_level) if val >= self._alpha)
