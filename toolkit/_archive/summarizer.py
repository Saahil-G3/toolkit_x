...
class Summarizer(Cleaner):
    ...
    def configure_summarizer_run(
        self, branch_name=None, df_name=None, make_df_dir=True
    ):
    ...
    def _initialize_summarizer_paths(self, make_df_dir=True):
        ...
        self._dirs["summarizer_results"] = (
            self._dirs["summarizer_root"] / f"{self.run_id}"
        )
        self._dirs["missing_value_report"] = (
            self._dirs["summarizer_results"] / f"missing_value_report"
        )
        self._dirs["summary_report"] = (
            self._dirs["summarizer_results"] / f"summary_report"
        )
        self._dirs["normality_report"] = (
            self._dirs["summarizer_results"] / f"normality_report"
        )
        ...
        self._paths["missing_value_report"] = (
            self._dirs["missing_value_report"]
            / f"missing_value_report_{self.df_name}.xlsx"
        )
    ...
    def create_missing_report(self):
        ...
        with pd.ExcelWriter(self._paths["missing_value_report"]) as writer:
            pd.DataFrame(self._missing_value_report).to_excel(
                writer, sheet_name="overview", index=False
            )
            pd.DataFrame(self._missing_identifier_report).to_excel(
                writer, sheet_name="missing_identifiers", index=False
            )
    ...
    def create_excel_summary_report(self, normality_test_type=None):
        ...
        with pd.ExcelWriter(self._paths["excel_summary_report"]) as writer:
            if self.num_col_names is not None:
                ...
                pd.DataFrame(self.num_col_summary).to_excel(
                    writer, sheet_name="numerical", index=False
                )

            if self.cat_col_names is not None:
                self._set_cat_col_summary()
                pd.DataFrame(self.cat_col_summary).to_excel(
                    writer, sheet_name="categorical", index=False
                )
