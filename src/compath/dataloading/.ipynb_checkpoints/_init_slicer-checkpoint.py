...
class InitSlicer:
    ...
    def _filter_coordinates(self, slice_key, show_progress=False):
        ...
        filtered_coordinates = []  # (coordinate, is_boundary)
        ...
        for x, y in iterator:
            ...
            if self.tissue_geom_prepared.intersects(box):
                if self.tissue_geom_prepared.contains(box):
                    filtered_coordinates.append(((x, y), False))
                else:
                    filtered_coordinates.append(((x, y), True))

        self.sph[slice_key]["filtered_coordinates"] = filtered_coordinates
        self.sample_using_tissue_geom = True
