...
def fill_polygon(polygon, mask_dims, device):
    x_points, y_points = torch.meshgrid(
        torch.arange(mask_dims[0], dtype=polygon.dtype, device=device),
        torch.arange(mask_dims[1], dtype=polygon.dtype, device=device),
        indexing="ij"
    )
    ...
    polygon_x1, polygon_y1 = polygon[:, 0], polygon[:, 1]
    polygon_x2, polygon_y2 = torch.roll(polygon_x1, shifts=-1, dims=0), torch.roll(polygon_y1, shifts=-1, dims=0)

    condition = (polygon_y1 > y_points[:, None]) != (polygon_y2 > y_points[:, None])
    intersect_x = (y_points[:, None] - polygon_y1) * (polygon_x2 - polygon_x1) / (polygon_y2 - polygon_y1) + polygon_x1

    mask = (condition & (intersect_x > x_points[:, None])).sum(dim=1).bitwise_and_(1)

    return mask.view(mask_dims)
